...
if category_item_limits_time_block_minutes:
    category_item_limit_scale = max(
        1, round(practice_time_minutes / category_item_limits_time_block_minutes)
    )
    ...
    return pd.DataFrame(
        {
            "time": df.apply(
                lambda row: max(
                    1, random.randrange(int(row.min_time), int(row.max_time) + 1)
                ),
                axis=1,
            )
        },
...
while session.time.sum() < practice_time_minutes and len(items) > 0:
    ...
    for category, group in items.groupby("category"):
        ...
        if (
            not args.ignore_category_max_counts
            and not np.isnan(max_category)
            and current_items_in_category >= max_category
        ):
            print(
                'Category "{0}" reached maximum item count ({1})'.format(
...
display_session.to_csv(
    session_output_file, index=False, index_label=False, encoding="utf-8"
)
